...
SRF3ToSpotify imports the play history of one or more SRF radio channels, stores each broadcast event, presents play-frequency rankings, matches logical radio songs to Spotify tracks, and keeps one Spotify playlist aligned with a configurable ranking.
...
| Spotify | Authorize the owner, search tracks, and manage the target playlist |
...
- Creation or update of a single managed Spotify playlist.
...
| Synchronization | Top-50 playlist completes within 2 minutes when Spotify is not rate-limiting |
